...
export function SaveStateIndicator({
  ...
  if (state === 'dirty') {
    return (
      <span aria-label="Unsaved changes" className={cn('flex items-center gap-1.5', className)}>
  ...
  if (state === 'saving') {
    return (
      <span
        role="status"
        aria-live="polite"
        className={cn('flex items-center gap-2 text-caption text-muted-foreground', className)}
      >
    ...
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          <span
            role="status"
            className={cn('flex items-center gap-2 text-caption text-warning', className)}
          >
  ...
  return (
    <span
      role="alert"
      aria-live="assertive"
      className={cn('flex items-center gap-2 text-caption text-destructive', className)}
    >
      <XCircle aria-hidden="true" className="size-3.5" />
      Couldn&rsquo;t save
      {onRetry && (
        <Button size="sm" variant="ghost" className="h-7 px-2 py-1 text-caption" onClick={onRetry}>
