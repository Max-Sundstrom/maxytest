...
export function BlockCatalogPanel({ studyId, workspaceId }: BlockCatalogPanelProps) {
  ...
  //   1. If a sidebar inline `+` set `catalogInsertPosition`, honour that
  //      exact slot (clamped to [1, thanksIdx] so welcome stays first
  //      and thanks stays last).
  //   2. Otherwise default to "before thanks" — the historical behaviour
  //      of the bottom `+ Добавить блок` button.
  const thanksIdx = blocks.findIndex((b) => b.type === 'thanks');
  const defaultPosition = thanksIdx >= 0 ? thanksIdx : blocks.length;
  const insertPosition = (() => {
    if (forcedPosition === null || forcedPosition === undefined) return defaultPosition;
