/**
 * <ScaleFocusedReport /> — focused-block card for `scale`-typed blocks (BLK-05,
 * 5/7/10-point numeric scale).
 *
 * Phase 4 / Plan 04-04 Task 2. Consumes the `ScaleStats` produced by
 * `@/lib/analytics/scale-stats` (histogram + mean/median/std/n).
 *
 * Visual:
 *   - Header pattern from PrototypeFocusedReport (position + chip + title + N).
 *   - 3-tile stat grid: Среднее / Медиана / Σ (population std).
 *   - Vertical histogram (bottom-up, flex: 1 columns) — bar height
 *     proportional to max count, with mono-formatted index + count labels.
 *   - Endpoint labels under histogram (left-aligned min, right-aligned max).
 *
...
import type { ScaleContent } from '@/lib/blocks/schemas';
...
export interface ScaleFocusedReportProps {
  block: Block;
  stats: ScaleStats;
  position: number;
...
export function ScaleFocusedReport({
  block,
  stats,
  position,
  validSessionCount,
  ...
  onResetFilters,
  publicMode = false,
}: ScaleFocusedReportProps): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {/* Endpoint labels — only render when designer provided them. */}
        {(content.endpointMinLabel || content.endpointMaxLabel) && (
          <div
            style={{
              display: 'flex',
